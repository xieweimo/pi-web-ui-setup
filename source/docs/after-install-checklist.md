...
- 版本：`pi 0.85.1` + `pi-web-ui 0.86.2`
- 5 个功能补丁：逐条成本、切模型即时刷新、历史列表 fork 去重、断连恢复浮层、红色脉冲停止按钮
- 2 个界面插件：`⚡额度`（ChatGPT 订阅额度 + 人民币成本）、`🛠PIwork`（顶栏 `⤴同步`）
...
| 顶栏没有 `⚡额度` / `🛠PIwork` | 设置 →「界面插件」里是否被隐藏；刷新页面 |
