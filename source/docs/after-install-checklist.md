...
- 版本：`pi 0.87.1` + `pi-web-ui 0.95.0`
- 4 个功能补丁：切模型即时刷新、原生菜单项顶部独立按钮、插件入口首帧缓存、红色脉冲停止按钮。
  （快捷短语排队用上游内建的**右键发送**；逐条成本、fork 链尾去重、永久忽略最近项目三项已由上游内建；断连浮层已删除，恢复入口统一为顶栏「🔄 重连」）
- 3 个界面插件：`⚡额度`（ChatGPT 订阅额度 + 人民币成本）、`⤴同步`、`🔄重连`；临时问答统一使用上游原生「临时对话」
...
1. 顶栏能看到 `⚡额度`、`⤴同步`、`🔄重连`，临时提问入口使用上游原生 `🎭临时对话`
...
| 顶栏没有 `⚡额度` / `⤴同步` / `🔄重连` | 设置 →「界面插件」里是否被隐藏；刷新页面 |
