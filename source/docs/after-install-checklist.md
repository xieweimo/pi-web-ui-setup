...
- 版本：`pi 0.85.1` + `pi-web-ui 0.94.1`
- 3 个功能补丁（pi-web-ui 0.94.1）：切模型即时刷新、原生菜单项顶部独立按钮、红色脉冲停止按钮。
  （快捷短语排队用上游内建的**右键发送**；逐条成本、fork 链尾去重、永久忽略最近项目三项已由上游内建；断连浮层已删除）
  （逐条成本、fork 链尾去重、永久忽略最近项目三项已由上游 0.93/0.94 内建；断连浮层已在 0.94.1 删除，恢复入口统一为顶栏「🔄 重连」）
